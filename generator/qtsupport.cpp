...
/**
 * Lookup candidates function of name \a methodName within the QObject
 * derivative \a objClass its bases, or its private implementation
 */
static llvm::SmallVector<clang::CXXMethodDecl *, 10>
lookUpCandidates(const clang::CXXRecordDecl *objClass,
 ...
 * sender or the receiver \a method is an expression like SIGNAL(....)  or
 * SLOT(....)
 *
...
/**
 * Very similar to handleSignalOrSlot, but does not handle the fact that the
 * string might be in a macro and does the string contains the method name and
 * not the full signature \a obj is an expression to a type of an QObject (or
 * pointer to) that is the sender or the receiver \a method is an expression of
 * type char*
 *
...
void QtSupport::visitCallExpr(clang::CallExpr *e) {
  ...
  if (parentName == "QObject" && (methodDecl->getName() == "connect" ||
  ...
  if (parentName == "QNetworkAccessCache" &&
      methodDecl->getName() == "requestEntry") {
    if (e->getNumArgs() >= 3) {
      handleSignalOrSlot(e->getArg(1), e->getArg(2));
    }
  }
  if (parentName == "QDBusAbstractInterface" &&
...
void QtSupport::visitCXXConstructExpr(clang::CXXConstructExpr *e) {
  ...
  if (parent->getName() == "QSignalSpy" ||
      parent->getName() == "QSignalTransition") {
    if (e->getNumArgs() >= 2) {
      handleSignalOrSlot(e->getArg(0), e->getArg(1));
    }
  }
