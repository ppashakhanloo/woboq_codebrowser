...
private:
  ...
  void addReference(const std::string &ref, clang::SourceRange refLoc,
...
public:
   ...
   * In case the file is in an external project that needs a data-proj tag, the
   *proj string is set to the project name.
   **/
  std::string pathTo(clang::FileID From, clang::FileID To,
                     std::string *proj = nullptr);
  std::string pathTo(clang::FileID From, const clang::FileEntry *To);

  /**
   * Registers that the code in the given \a range refers to the specified
   * declaration. This will highlight the rights portion of the code and
   * registers it in the database.
   *
   * When the range.getEndLocation refers to an invalid location, this means
   * this is a virtual range with no associated token (e.g. implicit
   * conversions) which will be handled with an empty <span>.
   *
   * Only use typeRef for declarations (or definition), only use usedContext for
   * uses
   */
  ...
  // same, but for macro
  void registerMacro(const std::string &ref, clang::SourceLocation refLoc,
                     DeclType declType);

  // Class names, structs, objective C identifiers, main function
  void registerInterestingDefinition(clang::SourceRange range,
                                     clang::NamedDecl *decl);

  /**
   * Wrap the source range in an HTML tag
   */
  void annotateSourceRange(clang::SourceRange range, std::string tag,
