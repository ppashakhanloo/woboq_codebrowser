 ...
 */
template <class T> bool cxxDeclIndependent(const T *decl) {
...
ssize_t getFieldOffset(const clang::Decl *decl) {
  ...
  const clang::ASTRecordLayout &layout =
      decl->getASTContext().getASTRecordLayout(parent);
...
Annotator::Visibility Annotator::getVisibility(const clang::NamedDecl *decl) {
  ...
  case clang::ExternalLinkage:
    if (decl->getDeclContext()->isRecord() &&
        mainFID == sm.getFileID(sm.getSpellingLoc(
...
bool Annotator::generate(clang::Sema &Sema, bool WasInDatabase) {
  ...
  for (auto it : cache) {
    ...
    "'>" << filename << "</a>" title=\"Arguments: << " <<
    Generator::escapeAttr(args)   <<"\"" */
  ...
  for (const auto &it : references) {
    ...
    if (error_code) {
      std::cerr << "Error writing ref file " << filename << ": "
...
std::string Annotator::pathTo(clang::FileID From, clang::FileID To,
  ...
  if (pr_it->second->type == ProjectInfo::External) {
    generator(From).addProject(pr_it->second->name,
...
void Annotator::registerReference(clang::NamedDecl *decl,
  ...
  // When the end location is invalid, this is a virtual range with no matching
  // tokens (eg implicit conversion)
  ...
  if (!range.getBegin().isFileID()) { // macro expension.
    ...
    if (sm.getFileID(spel1) != FID || sm.getFileID(spel2) != FID) {
      ...
      if (visibility == Visibility::Global) {
        ...
        addReference(getReferenceAndTitle(decl).first, range, type, declType,
                     typeText, decl);
      }
      return;
    }
  ...
  // Do some additional escaping for filenames (e.g., ':' is not valid on
  // Windows)
  llvm::SmallString<64> refFilenameBuffer;
  auto refFilename =
      Generator::escapeAttrForFilename(escapedRef, refFilenameBuffer);
...
void Annotator::addReference(const std::string &ref, clang::SourceRange refLoc,
                             TokenType type, DeclType dt,
  ...
  if (type == Ref || type == Member || type == Decl || type == Call ||
    ...
    if (dt < Use) {
      ...
      clang::FullSourceLoc fulloc(decl->getSourceRange().getBegin(),
                                  getSourceMgr());
      commentHandler.decl_offsets.insert(
          {fulloc.getSpellingLoc(), {ref, true}});
      if (auto parentStruct =
...
void Annotator::registerMacro(const std::string &ref,
                              clang::SourceLocation refLoc, DeclType declType) {
...
void Annotator::reportDiagnostic(clang::SourceRange range,
  ...
  llvm::SmallString<64> buffer;
  annotateSourceRange(range, "span",
                      "class='" % clas % "' title=\"" %
 ...
 * HTMLRewrite.cpp from the clang 3.1 from The LLVM Compiler Infrastructure
      ...
      if (startOfLine) {
        ...
        commentHandler.handleComment(
            *this, generator, Sema, BufferStart, CommentBegin, CommentLen,
            Tok.getLocation(),
            Tok.getLocation().getLocWithOffset(nl_it -
                                               (BufferStart + NonCommentBegin)),
