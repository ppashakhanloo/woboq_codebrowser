...
struct BrowserASTVisitor : clang::RecursiveASTVisitor<BrowserASTVisitor> {
  ...
  bool VisitFunctionDecl(clang::FunctionDecl *d) {
    ...
    bool isDefinition =
        d->isThisDeclarationADefinition() || d->hasAttr<clang::AliasAttr>();
  ...
  bool VisitMemberExpr(clang::MemberExpr *e) {
    ...
    annotator.registerUse(e->getMemberDecl(), range,
                          isMember(e->getMemberDecl()) ? Annotator::Member
                                                       : Annotator::Ref,
  ...
  bool VisitDesignatedInitExpr(clang::DesignatedInitExpr *e) {
    ...
    llvm::ArrayRef<clang::DesignatedInitExpr::Designator> designators{
        e->designators_begin(), e->designators_end()};
#else
    ...
    for (auto it : designators) {
      if (it.isFieldDesignator()) {
        if (auto decl = it.getField()) {
          annotator.registerUse(decl, it.getFieldLoc(), Annotator::Ref,
                                currentContext, Annotator::Use_Write);
        }
      }
    }
    return true;
  ...
  bool VisitTagTypeLoc(clang::TagTypeLoc TL) {
    ...
    annotator.registerUse(
        TL.getDecl(), range.getBegin(), Annotator::Type, currentContext,
  ...
  bool VisitCXXConstructExpr(clang::CXXConstructExpr *ctr) {
    if (clang::CXXConstructorDecl *decl = ctr->getConstructor()) {
      ...
      if (parenLoc.isValid()) {
        ...
        annotator.registerUse(
            decl, {ctr->getLocation(), clang::SourceLocation{}}, Annotator::Ref,
            currentContext, Annotator::Use_Call);
      }
    }
  ...
  bool VisitCallExpr(clang::CallExpr *e) {
    ...
    if (decl && !llvm::isa<clang::CXXOperatorCallExpr>(e)) {
      // Don't handle CXXOperatorCallExpr because it is obvious for operator= +=
      // and so on. And also because of the wierd rules regarding the member
      // operators and their number of arguments
      for (unsigned int i = 0;
           decl && i < e->getNumArgs() && i < decl->getNumParams(); ++i) {
  ...
  bool VisitGotoStmt(clang::GotoStmt *stm) {
    if (auto label = stm->getLabel()) {
      annotator.registerReference(label, stm->getLabelLoc(), Annotator::Label,
                                  Annotator::Use, {}, currentContext);
    }
    return true;
  }
  ...
  bool TraverseDecl(clang::Decl *d) {
    ...
    if (auto v = llvm::dyn_cast<clang::VarDecl>(d)) {
      if (v->getInit() && !expr_stack.topExpr) {
        ...
        auto t = v->getType();
        expr_stack.topType = (t->isReferenceType() &&
                              !t.getNonReferenceType().isConstQualified())
  ...
  bool TraverseStmt(clang::Stmt *s) {
    ...
    if (e) {
      ...
      if (auto i = llvm::dyn_cast_or_null<clang::IfStmt>(s)) {
        ...
        if (auto f =
                llvm::dyn_cast_or_null<clang::FunctionDecl>(currentContext)) {
  ...
  bool shouldUseDataRecursionFor(clang::Stmt *S) {
    // We need to disable this data recursion feature otherwise this break the
    // detection of parents (expr_stack).  Especially for the CaseStmt
    return false;
  }
#endif
  ...
  Annotator::DeclType classify() {
    ...
    for (auto expr : expr_stack) {
      ...
      if (auto op = llvm::dyn_cast<clang::CXXOperatorCallExpr>(expr)) {
        // Special case for some of the CXXOperatorCallExpr to check if it is
        // Use_Write Anything else goes through normal CallExpr
        auto o = op->getOperator();
        if (o == clang::OO_Equal ||
            (o >= clang::OO_PlusEqual && o <= clang::OO_PipeEqual) ||
            (o >= clang::OO_LessLessEqual &&
             o <= clang::OO_GreaterGreaterEqual) ||
