...
void PreprocessorCallback::MacroExpands(const clang::Token &MacroNameTok,
  ...
  std::string ref =
      llvm::Twine("_M/", MacroNameTok.getIdentifierInfo()->getName()).str();
  ...
  if (sm.getMainFileID() != defFID) {
    annotator.registerMacro(ref, MacroNameTok.getLocation(),
                            Annotator::Use_Call);
  }
  ...
  annotator.generator(FID).addTag("a", tag, sm.getFileOffset(loc),
                                  MacroNameTok.getLength());
}
...
void PreprocessorCallback::MacroDefined(const clang::Token &MacroNameTok,
  ...
  annotator.generator(FID).addTag(
      "dfn", "class=\"macro\" id=\"" % ref % "\" data-ref=\"" % ref % "\"",
      sm.getFileOffset(loc), MacroNameTok.getLength());
}

void PreprocessorCallback::MacroUndefined(
    const clang::Token &MacroNameTok, PreprocessorCallback::MyMacroDefinition MD
...
bool PreprocessorCallback::FileNotFound(
    llvm::StringRef FileName, llvm::SmallVectorImpl<char> &RecoveryPath) {
  if (!recoverIncludePath)
    return false;
  clang::SourceLocation currentLoc =
      static_cast<clang::Lexer *>(PP.getCurrentLexer())->getSourceLocation();
  auto &SM = annotator.getSourceMgr();
  const clang::FileEntry *entry =
      SM.getFileEntryForID(SM.getFileID(currentLoc));
  if (!entry || llvm::StringRef(entry->getName()).empty())
    return false;
  std::string recovery =
      annotator.projectManager.includeRecovery(FileName, entry->getName());
  if (recovery.empty() || !llvm::StringRef(recovery).endswith(FileName))
    return false;
  RecoveryPath.clear();
  RecoveryPath.append(recovery.begin(),
                      recovery.begin() + recovery.size() - FileName.size());
  currentLoc.dump(SM);
  llvm::errs() << " WARNING: File not found '" << FileName
               << "'. Recovering using "
...
void PreprocessorCallback::Defined(const clang::Token &MacroNameTok,
                                   MyMacroDefinition MD,
                                   clang::SourceRange Range) {
  clang::SourceLocation loc = MacroNameTok.getLocation();
  if (!loc.isValid() || !loc.isFileID())
...
void PreprocessorCallback::HandlePPCond(clang::SourceLocation Loc,
  ...
  annotator.generator(FID).addTag(
      "span",
      ("data-ppcond=\"" + clang::Twine(SM.getExpansionLineNumber(IfLoc)) + "\"")
          .str(),
