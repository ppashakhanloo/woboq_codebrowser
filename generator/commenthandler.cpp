      ...
      if (Next.is(clang::tok::coloncolon)) {
        ...
        if (TemplateKind == clang::TNK_Non_template) {
          ...
          clang::Sema::NestedNameSpecInfo nameInfo(II, Tok.getLocation(),
                                                   Next.getLocation());
          if (Sema.ActOnCXXNestedNameSpecifier(Sema.getScopeForContext(TuDecl),
                                               nameInfo, false, SS))
#else
...
struct CommentHandler::CommentVisitor
  ...
  void visitBlockCommandComment(const clang::comments::BlockCommandComment *C) {
    ...
    tag("command", {C->getSourceRange().getBegin(),
                    nameRange.getEnd().getLocWithOffset(-1)});
    for (unsigned int i = 0; i < C->getNumArgs(); ++i)
      tag("arg", C->getArgRange(i));
  ...
  // *C); void visitTParamCommandComment(const
  // clang::comments::TParamCommandComment *C);
  /*void visitVerbatimBlockComment(const clang::comments::VerbatimBlockComment
  *C) { Base::visitVerbatimBlockComment(C);
      FIXME
  ...
  void visitVerbatimLineComment(const clang::comments::VerbatimLineComment *C) {
    ...
    // overlap with the command.  And also includes the end of line, which is
    // useless.
...
private:
  ...
  void parseEnumValue(const clang::comments::BlockCommandComment *C) {
    ...
    auto len =
        range.getEnd().getRawEncoding() - range.getBegin().getRawEncoding() + 1;
    auto ctn = std::string(
        annotator.getSourceMgr().getCharacterData(range.getBegin()), len);
    SubDocs.push_back({std::move(ref), Doc{std::move(ctn), range.getBegin()}});
  }
};

static void handleUrlsInComment(Generator &generator, llvm::StringRef rawString,
                                int commentStart) {
  std::size_t pos = 0;
  while ((pos = rawString.find("http", pos)) != llvm::StringRef::npos) {
    ...
    if (begin != 0 &&
        llvm::StringRef(" \t/*[]()<>|:\"'{}").find(rawString[begin - 1]) ==
            llvm::StringRef::npos) {
      // the URL need to be the first character, or follow a space or one of the
      // character
      continue;
    }
    ...
    llvm::StringRef urlChars =
        "-._~:/?#[]@!$&'()*+,;=%"; // chars valid in the URL
    ...
    // Don't end with a closing parenthese or bracket unless the URL contains an
    // opening one (e.g. wikipedia urls)
    auto candidate = rawString.substr(begin, pos - begin);
    if (rawString[pos - 1] == ')' &&
        candidate.find('(') == llvm::StringRef::npos)
      pos--;
    if (rawString[pos - 1] == ']' &&
        candidate.find('[') == llvm::StringRef::npos)
      pos--;
  ...
  if ((rawString.ltrim().startswith("/**") &&
...
#endif
    {
...
#endif
      clang::comments::Lexer lexer(PP.getPreprocessorAllocator(),
                                   PP.getDiagnostics(), traits, commentLoc,
                                   bufferStart + commentStart,
      ...
      if (!visitor.DeclRef.empty()) {
        ...
        docs.insert(
            {std::move(visitor.DeclRef), {rawString.str(), commentLoc}});
