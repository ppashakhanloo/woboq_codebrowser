...
// ATTENTION: Keep in sync with ECMAScript function of the same name in .js
// files and `escapeAttrForFilename` generator.cpp
...
#ifdef _WIN32
  // Make sure we use forward slashes to make sure folder detection works as
  // expected everywhere
...
std::error_code create_directories(const llvm::Twine &path) {
  ...
  auto defaultPerms =
      perms::all_all & ~perms::group_write & ~perms::others_write;
